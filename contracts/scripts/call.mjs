...
const algodClient = new algosdk.Algodv2(process.env.ALGOD_TOKEN, algod_address, process.env.ALGOD_PORT);
...
const abi = JSON.parse(fs.readFileSync(path.join(__dirname, process.argv[3]), 'utf8'));

const contract = new algosdk.ABIContract(abi);
const suggestedParams = await algodClient.getTransactionParams().do();

if (process.argv[4] === 'delete') {
  const txn = algosdk.makeApplicationDeleteTxn(creator.addr, suggestedParams, app_id);
  ...
  console.log(
    `Calling ${app_id} on ${process.env.NETWORK} - method "${process.argv[4]}" with ${
